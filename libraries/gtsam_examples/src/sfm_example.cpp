...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < poses.size(); ++i) {
    ...
    for (size_t j = 0; j < points.size(); ++j) {
      ...
      graph.emplace_shared<GenericProjectionFactor<gtsam::Pose3, gtsam::Point3, gtsam::Cal3_S2>>(
  ...
  graph.addPrior(gtsam::Symbol('l', 0), points[0],
  ...
  for (size_t j = 0; j < points.size(); ++j) {
    gtsam::Point3 corrupted_point = points[j] + Point3(-0.25, 0.20, 0.15);
    initialEstimate.insert<Point3>(gtsam::Symbol('l', j), corrupted_point);
  }
  initialEstimate.print("Initial Estimates:\n");

  /* Optimize the graph and print results */
  gtsam::Values result =
      gtsam::DoglegOptimizer(graph, initialEstimate).optimize();
  result.print("Final results:\n");
  cout << "initial error = " << graph.error(initialEstimate) << endl;
  cout << "final error = " << graph.error(result) << endl;
  gtsam::Marginals marginals(graph, result);
  gtsam::Matrix cov_mat = marginals.marginalCovariance(gtsam::Symbol('x',1));

  std::cout << "Square root information matrix " << std::endl;
  std::cout << marginals.marginalInformation(Symbol('x', 2)) << std::endl;
  std::cout << " -------------------------------- " << std::endl;
  std::cout << marginals.marginalInformation(Symbol('x', 3)) << std::endl;
  std::cout << " -------------------------------- " << std::endl;
  std::cout << marginals.marginalInformation(Symbol('x', 3)) << std::endl;
